...
for dirname, commit_date in subdirs_with_dates:
    ...
    # _summary.md is written by the summarizer subagent during research, not here.
    # See .claude/agents/summarizer.md and .claude/skills/research/SKILL.md.
    if not summary_path.exists():
        raise RuntimeError(
            f"{dirname}/_summary.md is missing. The summarizer subagent writes it "
            f"before the research PR is opened; see .claude/agents/summarizer.md"
        )

    description = summary_path.read_text().strip()
    if not description:
        raise RuntimeError(f"{dirname}/_summary.md is empty")
    print(description)
...
### [Maintaining Dotfiles Across Multiple Hosts on macOS and Linux](https://github.com/daftdoki/research/tree/main/dotfiles-multi-host-management#readme) (2026-09-12 08:31)
...
### [Presenting a USB Drive to a Computer, Backed by a Network Drive](https://github.com/daftdoki/research/tree/main/usb-drive-backed-by-network#readme) (2026-06-03 19:36)
...
HRS HLX-090 Helix isolators are fully compatible with CH Precision L10, P10, D1.5, C1.2, and T1 units when placed on HRS SXR racks and M3X bases, provided they are positioned at the four chassis corners where the original feet reside. The casework is engineered to bear weight only at these corners, and replacing factory spikes with Helix pucks is recommended—especially on granite-topped M3X bases—due to superior vibration isolation and energy transfer on reflective surfaces. Factory feet on CH Precision units are uniquely placed for structural and vibration reasons, so Helix pucks must mimic the same layout; never run both spike and Helix drain paths simultaneously. Employ 4 Helix pucks per main unit (3 for T1), keeping spikes retracted or removed as needed for optimal performance.

Key points:
- Helix pucks replace CH spikes at corners, never both in contact
- Always position Helix pucks where factory feet were installed
- Designed for use atop HRS M3X bases with granite surfaces
- Corner-only support matches CH Precision's load and energy drain path design

Learn more about [HRS Helix](https://www.hrsaccessories.com/products/helix/) and [CH Precision electronics](https://www.ch-precision.com/).

### [Higher-Temp Filaments for 3D-Printed Filament-Dryer Accessories (Bambu Lab H2D)](https://github.com/daftdoki/research/tree/main/high-temp-filament-for-dryer-accessories#readme) (2026-04-21 17:50)

If you're looking to safely 3D-print accessories for use inside a filament dryer (running up to 65°C) using a Bambu Lab H2D, PETG-CF and PCTG are your best bets: both offer heat-deflection temperatures (HDT) exceeding those of standard PLA and PETG, print easily, and are indoor-safe. For higher thermal margins or load-bearing parts, Polycarbonate (PC, PC-FR) is a strong alternative, though it requires a slightly more demanding workflow. PAHT-CF (carbon fiber nylon) provides exceptional thermal and mechanical performance but is overkill unless you need its unique stiffness and strength, with printing challenges to match. For most dryer-accessory needs, PETG-CF combines simplicity, compatibility, and safety—making it the practical go-to.

Key Materials & Findings:
- **PETG-CF:** Easy to print, HDT ~80–85°C, indoor-safe; slight upgrade over PETG.
- **PCTG:** Minor improvement over PETG, similar printability and safety.
- **Polycarbonate/PC-FR:** HDT ~115–135°C, greatly expands thermal margin, moderate indoor safety.
- **PAHT-CF:** HDT >120°C, best mechanical properties, demanding workflow.

Reference tools: [Bambu Lab H2D](https://www.bambulab.com/en/product-element/h2d-hotend), [Filament dryer accessory projects](https://www.printables.com/model/409721-filament-dryer-gear-upgrades-for-eibos-xp)

### [3D-Printer Filament Appearances in Fusion 360](https://github.com/daftdoki/research/tree/main/fusion360-printer-filaments#readme) (2026-04-15 18:48)
